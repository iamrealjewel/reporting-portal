...
export default function StockSummaries() {
    ...
    useEffect(() => {
        document.title = `Inventory Ledger Analytics | DMS Portal`;
        setSearchQuery("");
    ...
    const fetchSummary = async () => {
        ...
        try {
            ...
            const { data } = await api.get(`/analytics/stock-summary?${params.toString()}`);
            setData(data);
        } catch (error) {
    ...
    const handleExport = () => {
        ...
        try {
            const exportData = data.map(item => {
                ...
                row["RETAILER VALUE"] = item._sum?.retailerAmount || 0;
                return row;
            });
            ...
            writeFile(wb, `stock_summary_by_${dimensions.join("_")}.xlsx`);
        ...
        <div className="space-y-6 pb-12 transition-all duration-300">
                ...
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h1 className="text-2xl font-black text-slate-900 tracking-tight flex items-center gap-3">
                            ...
                            Stock Analytical Summaries
                        </h1>
                        <p className="text-slate-500 text-sm mt-1 italic">Multi-level cross-tab aggregation and valuation reports for Inventory.</p>
                    </div>
                </div>
            </div>
            ...
            <div className="bg-white border border-slate-200 p-2.5 rounded-2xl shadow-sm flex flex-wrap items-center gap-2 overflow-visible sticky top-0 z-50">
                ...
                <MultiSelectFilter
                    ...
                    options={options?.stock?.divisions}
                />
                <MultiSelectFilter
                    label="Site Name"
                    selectedValues={filters.siteName}
                    onChange={(vals: string[]) => setFilters(f => ({ ...f, siteName: vals }))}
                    options={options?.stock?.siteNames}
                />
                <MultiSelectFilter
                    label="Category"
                    selectedValues={filters.category}
                    onChange={(vals: string[]) => setFilters(f => ({ ...f, category: vals }))}
                    options={options?.stock?.categories}
                />
                <MultiSelectFilter
                    label="Brand"
                ...
                <MultiSelectFilter
                    ...
                    options={options?.stock?.products}
                />
            ...
            <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
                <div className={`space-y-4 transition-all duration-300 ease-in-out transform origin-left ${isSidebarOpen ? 'lg:col-span-1 opacity-100 translate-x-0' : 'hidden opacity-0 -translate-x-full w-0 overflow-hidden'}`}>
                    ...
                    <Card className="rounded-2xl border-slate-200 border shadow-sm">
                        <CardHeader
                            className="bg-blue-50/30 border-b border-slate-100 p-4 cursor-pointer hover:bg-blue-50 transition-colors"
                            onClick={() => setIsPredefinedOpen(!isPredefinedOpen)}
                        >
                            <CardTitle className="text-xs font-black uppercase tracking-tighter text-blue-700 flex items-center justify-between">
                            ...
                            <Table>
                                <TableHeader className="bg-slate-50/50 sticky top-0 z-10 shadow-sm">
                                    <TableRow>
                                        ...
                                        <TableHead className="py-4 px-6 text-right text-[10px] font-black uppercase tracking-widest text-slate-500 whitespace-nowrap">Dealer Value (৳)</TableHead>
                                        <TableHead className="py-4 px-6 text-right text-[10px] font-black uppercase tracking-widest text-slate-500 whitespace-nowrap">Retailer Value (৳)</TableHead>
                                    </TableRow>
                                </TableHeader>
                                <TableBody>
                                    {loading ? (
                                        ...
                                        (() => {
                                            ...
                                            return paginatedData.map((item, idx) => (
                                                <TableRow key={idx} className="hover:bg-slate-50/30 transition-colors">
                                                    ...
                                                    <TableCell className="py-2.5 px-6 text-right font-mono text-[11px] text-slate-600 whitespace-nowrap bg-slate-50/30">
                                                        {(item._sum?.ltrKg || 0).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })} <span className="text-[9px] text-slate-400">ltr/kg</span>
                                                    </TableCell>
                                                    <TableCell className="py-2.5 px-6 text-right font-mono text-[11px] font-black text-blue-600 whitespace-nowrap bg-blue-50/10">
                                                        ৳{(item._sum?.dealerAmount || 0).toLocaleString()}
                                                    </TableCell>
                                                    <TableCell className="py-2.5 px-6 text-right font-mono text-[11px] font-black text-blue-700 whitespace-nowrap bg-blue-50/30">
                                                        ৳{(item._sum?.retailerAmount || 0).toLocaleString()}
                                                    </TableCell>
                                                </TableRow>
                                            ));
                                        })()
